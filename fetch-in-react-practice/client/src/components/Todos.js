...
export default function Todos() {
  ...
  async function toggleCompleted(todoId) {
    const clickedTodo = todos.find((todo) => todo.todoId === todoId );
    fetch(`/api/todos/${todoId}`, {
     method: 'PATCH',
     headers: { 'Content-Type': 'application/json' },
     body: JSON.stringify({isCompleted: !clickedTodo.isCompleted})
     })
      .then(response => {
       if (!response.ok) {
       ...
       .then(response => {
        const updateTodo = todos.map((currentTodo) => currentTodo.todoId === response.todoId ? response : currentTodo);
        setTodos(updateTodo);
      })
       .catch(err => {
         setError(err);
